# -*- coding: utf-8 -*-
"""loan_default.py - Final Code with Theme 2 (Professional Dark) and Layout Changes"""
...
def load_data():
    try:
        ...
        df = pd.read_excel('Loan_Prerna.xlsx')
        return df
...
if df is not None:
    ...
    # --- MIDDLE ROW: THE DRIVERS (Bar Charts - Theme 2 Applied) ---
    ...
    with row2_col1:
        ...
        fig_emp = px.bar(emp_risk, x='Employment_Type', y='Default Rate (%)',
                         ...
                         color='Default Rate (%)', 
                         color_continuous_scale='Blues', # <--- THEME 2: BLUES
                         template='plotly_dark') # <--- THEME 2: DARK TEMPLATE
    ...
    # --- CONFUSION MATRIX ANALYSIS (Layout Change: Stacked) ---
    st.subheader("Model Performance: Confusion Matrix")
    
    # Use a single column for the matrix display for maximum width utilization
    ...
    # Center the matrix display within the available space
    st.dataframe(cm_df, use_container_width=True)
    
    st.markdown("##### Understanding the Confusion Matrix Values") # Full width explanation
    st.markdown(f"""
    ...
    * **False Negative (FN): {FN}** - The model **Incorrectly** predicted **No Default** when the customer *did* default. This is a **Type II Error** (Major Risk: Approving a loan that will default).
    ...
    # --- BOTTOM ROW: THE PROFILE (Layout Change: Insight Wider) ---
    st.subheader("The Profile: High Risk Identification")

    # Swapped column ratio: Insight (2 parts) and Table (1 part)
